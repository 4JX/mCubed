    ...
    fn show(ui: &mut Ui, images: &ImageTextures) {
        Self::settings_section(ui, &images.settings, "General", |ui| {
            ui.label("Mods folder path")
                .on_hover_text("The path to the current mods folder of your Minecraft installation");

            if ui.button(CONF.lock().mod_folder_path.display().to_string()).clicked() {
                ...
                if let Some(folder) = folder {
                    SettingsBuilder::from_current().mod_folder_path(folder).apply();
                }
            }
        });
    }
